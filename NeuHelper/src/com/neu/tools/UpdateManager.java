...
public class UpdateManager {
	...
	public UpdateManager(Context context,boolean isFirstEnter) {
	...
	private boolean isUpdate() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					HttpPost httpPost = new HttpPost(ConstantUtils.forNewVersion);
		...
		try {
			...
			if (null != mHashMap) {				
				String serviceCode = String.valueOf(mHashMap.get("version"));
	
				// 版本判断
				if (Double.parseDouble(serviceCode)>Double.parseDouble(versionCode)) {
					return true;
				}
			}
		return false;
